...
function CloudSoftware() {
  ...
  return (
   ...
   <div className='mt-[8.19rem]'>
    ...
    <p className='text-[#696984] text-[1.5rem] font-normal leading-[2.7rem] text-center w-[52.3125rem] mx-auto mt-[1.25rem]'>TOTC is one powerful online software suite that combines all the tools needed to run a successful school or office.</p>
    <div className='ml-[14.06rem] mt-[2.44rem] flex gap-[3.75rem] '>
        {Cards.map((info,index)=>(
        <div className='bg-[#fff] card relative  top-32  w-[28.125rem] h-[26.875rem]'>
            <div className='flex justify-center absolute left-[30%] -top-20'>{info.mainImg}</div>
            <h2 className={`text-[#2F327D] text-center text-[1.875rem] font-medium leading-normal w-[20.3125rem] mx-auto pt-[7.62rem] ${index === 2 ? 'mt-[2rem] mb-12' : 'mt-[1rem]'}`}>{info.heading}</h2>
            <p className={`text-[#696984] text-center text-[1.25rem] font-normal leading-[2.25rem] w-[22rem] mx-auto mt-[1.5rem] ${index === 2 ? 'w-[20rem]' : ''}`}>{info.paragraph}</p>
        </div>
        ))}
    </div>
   </div>
